...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!emailApiKey) {
    console.error('EMAIL_API_KEY/RESEND_API_KEY not set — cannot send admin-triggered emails.');
    return NextResponse.json({
      success: false,
      error: 'Email sending is not configured. Add EMAIL_API_KEY (Resend) to .env.local/server environment and restart Next.js.'
    }, { status: 503 });
  }
