...
export async function POST(req:NextRequest){
 try{const body=normalizeProgramBody(await req.json());if(!body.name)return NextResponse.json({success:false,error:'Program name is required.'},{status:400});const admin=getSupabaseAdmin();const row=safariToRow(body as any,body.locale);const {data,error}=await admin.from('safari_programs').upsert(row,{onConflict:'slug,locale'}).select().single();if(error)throw new Error(error.message);return NextResponse.json({success:true,program:rowToSafari(data)});}catch(e){return NextResponse.json({success:false,error:e instanceof Error?e.message:'Failed to save program'},{status:500});}}

export async function DELETE(req:NextRequest){try{const body=await req.json();const admin=getSupabaseAdmin();const {error}=await admin.from('safari_programs').delete().eq('slug',String(body.slug)).eq('locale',normalizeLocale(body.locale));if(error)throw new Error(error.message);return NextResponse.json({success:true});}catch(e){return NextResponse.json({success:false,error:e instanceof Error?e.message:'Failed to delete program'},{status:500});}}
