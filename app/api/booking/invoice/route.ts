...
export async function POST(req: NextRequest) {
  try {
    ...
    const { base64, dataUrl } = await generatePremiumInvoicePDF(booking, locale);
    ...
    if (emailApiKey) {
      ...
      const html = `
        <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#1f2937">
          <div style="background:#0e7490;padding:24px;border-radius:14px 14px 0 0;color:#fff">
            <h1 style="margin:0;font-size:22px">Your Provisional Invoice</h1>
            <p style="margin:6px 0 0;color:#dff7fb">Bahari Asili Safaris · Booking ${escapeHtml(bookingRef)}</p>
          </div>
          <div style="padding:24px;background:#fff;border:1px solid #e2e8f0">
            <p style="margin-top:0">Dear ${fullName},</p>
            <p>Thank you for your booking request for <strong>${safeTour}</strong>.</p>
            <p>We have generated your provisional invoice automatically from the tour and traveller details you selected. The PDF includes the itinerary, description, inclusions, exclusions, travel date, guests and booking information.</p>
            <p style="margin-bottom:0">Final pricing, availability and any accommodation or pickup changes will be confirmed by our team before payment.</p>
          </div>
          <div style="background:#1f2937;padding:18px;border-radius:0 0 14px 14px;text-align:center;color:#cbd5e1;font-size:12px">
