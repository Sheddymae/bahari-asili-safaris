    ...
    <section className="py-20 lg:py-28 bg-sand-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {featuredDestinations.map((d) => (
            <Link
              ...
              prefetch
              className="group relative rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-500 h-64 sm:h-80 hover:-translate-y-1"
            >
              <Image
                src={d.heroImage}
                alt={`${d.name} safari destination${d.country ? `, ${d.country}` : ''}`}
                fill
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
                ...
                <span className="block mt-1.5 font-inter text-xs text-white/80 group-hover:text-white transition-colors line-clamp-2">{d.tagline}</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="text-center mt-10">
          <Link href="/destinations" prefetch className="inline-flex items-center gap-2 font-inter font-semibold text-sm text-ocean-700 hover:text-ocean-800 transition-colors">
            {e.destCta}
