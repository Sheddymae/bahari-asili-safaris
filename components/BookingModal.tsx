...
import { countryFromSearch, isCountry } from '@/lib/countries';

const WHATSAPP_NUMBER = '254101923355';
type Status = 'idle' | 'loading' | 'success' | 'error';
type Step = 1 | 2 | 3;
interface BookingModalProps { isOpen: boolean; onClose: () => void; selectedTour?: string; }
interface FormState { firstName: string; lastName: string; email: string; whatsapp: string; nationality: string; adults: string; children: string; arrivalDate: string; safari: string; message: string; }
...
export function formatKidsAges(ages: number[] | null | undefined, locale: string): string { if (!ages?.length) return ''; const years = locale === 'it' ? 'anni' : locale === 'fr' ? 'ans' : locale === 'es' ? 'años' : locale === 'de' ? 'Jahre' : locale === 'ar' ? 'سنوات' : locale === 'zh' ? '岁' : locale === 'sw' ? 'miaka' : 'yrs'; return `${ages.join(', ')} ${years}`; }
...
export default function BookingModal({ isOpen, onClose, selectedTour }: BookingModalProps) {
  ...
  useEffect(() => { setKidsAges((previous) => previous.length === childCount ? previous : childCount < previous.length ? previous.slice(0, childCount) : [...previous, ...Array(childCount - previous.length).fill('')]); setKidsAgesError(false); }, [childCount]);
  useEffect(() => {
    if (!user || !isOpen) return; const meta = user.user_metadata || {}; const parts = String(meta.full_name || '').trim().split(/\s+/); const savedNationality = String(meta.nationality || '').trim();
    const normalizedNationality = countryFromSearch(savedNationality) || (isCountry(savedNationality) ? savedNationality : '');
  ...
  return <><div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><button aria-label="Close" className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} /><div role="dialog" aria-modal="true" aria-labelledby="booking-modal-title" className="relative bg-white rounded-3xl shadow-hero w-full max-w-lg max-h-[92vh] overflow-y-auto z-10"><div className="sticky top-0 bg-white rounded-t-3xl border-b border-border px-5 sm:px-6 py-5 z-10"><div className="flex items-start justify-between gap-4"><div><h2 id="booking-modal-title" className="font-poppins font-bold text-xl text-foreground">{t.booking.title}</h2><p className="font-inter text-muted-foreground text-sm mt-1">{copy.step} {step} {copy.of} 3 · {copy.steps[step - 1]}</p></div><button onClick={onClose} aria-label="Close" className="w-9 h-9 shrink-0 rounded-full bg-muted flex items-center justify-center"><X className="w-4 h-4" /></button></div><div className="grid grid-cols-3 gap-1.5 mt-4">{[1, 2, 3].map((n) => <div key={n} className={`h-1.5 rounded-full ${n <= step ? 'bg-ocean-700' : 'bg-border'}`} />)}</div></div>
  {status === 'success' ? <div className="px-6 py-10"><div className="text-center mb-5"><CheckCircle className="w-14 h-14 text-ocean-700 mx-auto mb-3" /><h3 className="font-poppins font-bold text-xl">{t.booking.success}</h3></div><InquiryStatusDisplay bookingRef={bookingRef} firstName={form.firstName} email={form.email} whatsapp={form.whatsapp} emailSent={emailSent} status="pending" />{showAuthPrompt && <div className="mt-5 bg-ocean-50 border border-ocean-200 rounded-xl p-4 flex gap-3"><UserPlus className="w-5 h-5 text-ocean-700 shrink-0" /><div><p className="font-semibold text-sm">{t.bookingModal.createAccountTitle}</p><p className="text-xs text-muted-foreground mt-1 mb-3">{t.bookingModal.createAccountDesc}</p><button onClick={() => setAuthModalOpen(true)} className="rounded-lg bg-ocean-700 text-white px-4 py-2 text-sm font-semibold">{t.bookingModal.createAccountBtn}</button></div></div>}<div className="grid grid-cols-2 gap-3 mt-6"><a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${buildWhatsAppMsg(bookingRef)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] text-white py-3 font-semibold"><MessageCircle className="w-4 h-4" />WhatsApp</a><button onClick={onClose} className="rounded-xl bg-ocean-700 text-white py-3 font-semibold">OK</button></div></div> : <form onSubmit={submit} className="px-5 sm:px-6 py-6"><div className="mb-5 rounded-xl bg-sand-50 border border-border px-4 py-3 text-xs text-muted-foreground">{copy.required}</div>
