              ...
              return (
                <Link key={e.id} href={`/excursions/${e.id}`} prefetch className="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-card transition-shadow hover:shadow-card-hover">
                  <div className="relative h-52 w-full overflow-hidden">
                    ...
                    {e.popular && <div className="absolute left-3 top-3 rounded-full bg-safari-500 px-3 py-1 shadow-sm"><span className="text-[10px] font-bold uppercase tracking-wide text-white">{t.excursions.popular}</span></div>}
                    <div className="absolute bottom-0 left-0 right-0 p-5"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-widest text-white/70">{categoryLabel[e.category]}</span><h2 className="font-poppins text-xl font-bold leading-snug text-white">{content.name}</h2></div>
                  </div>
