      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 will-change-transform ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg py-3' : 'bg-white/10 backdrop-blur-md border-b border-white/20 py-5'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isMobileOpen && <div id="mobile-menu" className="xl:hidden mt-4 bg-white rounded-2xl shadow-xl border border-border overflow-hidden animate-in slide-in-from-top-2">
            ...
            <div className="border-t border-border px-4 py-3 space-y-2">{loading ? null : user ? <><div className="flex items-center gap-3 px-3 py-2"><div className="w-9 h-9 rounded-full bg-safari-500 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white" /></div><div className="min-w-0"><p className="font-inter font-semibold text-sm text-foreground truncate">{displayName}</p><p className="font-inter text-xs text-muted-foreground truncate">{user.email}</p></div></div><Link href="/dashboard" onClick={() => setIsMobileOpen(false)} className="block text-center font-inter text-sm font-semibold px-4 py-2.5 bg-ocean-700 text-white rounded-xl">{t.authSession.myDashboard}</Link><button onClick={signOut} className="w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-[#ef4444]/30 text-destructive rounded-xl">{t.nav.signOut}</button></> : <><button onClick={() => { setIsMobileOpen(false); setAuthModal({ open: true, mode: 'signin' }); }} className="block w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-border text-foreground rounded-xl">{t.nav.signIn}</button><button onClick={() => { setIsMobileOpen(false); setAuthModal({ open: true, mode: 'signup' }); }} className="block w-full text-center font-inter text-sm font-semibold px-4 py-2.5 bg-ocean-700 text-white rounded-xl">{t.nav.register}</button></>}</div>
            <div className="border-t border-border px-4 py-3 space-y-2">{user ? <><div className="flex items-center gap-3 px-3 py-2"><div className="w-9 h-9 rounded-full bg-safari-500 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white" /></div><div className="min-w-0"><p className="font-inter font-semibold text-sm text-foreground truncate">{displayName}</p><p className="font-inter text-xs text-muted-foreground truncate">{user.email}</p></div></div><Link href="/dashboard" onClick={() => setIsMobileOpen(false)} className="block text-center font-inter text-sm font-semibold px-4 py-2.5 bg-ocean-700 text-white rounded-xl">{t.authSession.myDashboard}</Link><button onClick={signOut} className="w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-[#ef4444]/30 text-destructive rounded-xl">{t.nav.signOut}</button></> : <><button onClick={() => { setIsMobileOpen(false); setAuthModal({ open: true, mode: 'signin' }); }} className="block w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-border text-foreground rounded-xl">{t.nav.signIn}</button><button onClick={() => { setIsMobileOpen(false); setAuthModal({ open: true, mode: 'signup' }); }} className="block w-full text-center font-inter text-sm font-semibold px-4 py-2.5 bg-ocean-700 text-white rounded-xl">{t.nav.register}</button></>}</div>
          </div>}
        </div>
      </nav>
