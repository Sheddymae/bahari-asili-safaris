...
export default function Navbar() {
  ...
  // Close mobile menu / user menu on Escape
  useEffect(() => {
  ...
  // Phase 2: real pages, not in-page anchors — Next.js <Link prefetch> gives
  // client-side (same-tab, no white-flash) navigation between them.
  const navLinks = useMemo(() => [
    { label: t.nav.tours, href: '/tours' },
    { label: t.nav.excursions, href: '/excursions' },
    { label: t.nav.destinations, href: '/destinations' },
    { label: t.nav.services, href: '/services' },
    { label: 'Build Your Safari', href: '/build-your-safari' },
              ...
              {navLinks.map(link => (
                <Link
                  ...
                  className={`nav-link font-inter font-medium text-sm transition-colors pb-0.5 cursor-pointer ${
                    isScrolled? 'text-foreground hover:text-ocean-700' : 'text-white/90 hover:text-white'
                  }`}
                >
                  {link.label}
                </Link>
                  ...
                  <div className="relative" onClick={e => e.stopPropagation()}>
                    ...
                    {isUserMenuOpen && (
                      <div className="absolute right-0 top-full mt-2 bg-white rounded-xl shadow-xl border border-border overflow-hidden z-50 min-w-[220px]">
                        ...
                        <Link
                          href="/account"
                          className="flex items-center gap-2 w-full px-4 py-3 text-sm font-inter text-foreground hover:bg-muted transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 w-full px-4 py-3 text-sm font-inter text-destructive hover:bg-[#ef4444]/10 transition-colors border-t border-border"
                        >
              ...
              <div className="px-4 py-3 space-y-1">
                {navLinks.map(link => (
                  <Link
                    ...
                    className="block py-2.5 px-3 font-inter font-medium text-foreground hover:text-ocean-700 hover:bg-sand-50 rounded-lg transition-colors cursor-pointer"
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <div className="border-t border-border px-4 py-3 space-y-2">
                {user? (
                  <>
                    ...
                    <button
                      onClick={signOut}
                      className="w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-[#ef4444]/30 text-destructive rounded-xl"
                    >
                      {t.nav.signOut}
                    </button>
                  </>
                  ...
                  <>
                    <button
                      ...
                      className="block w-full text-center font-inter text-sm font-medium px-4 py-2.5 border border-border text-foreground rounded-xl"
                    >
                      {t.nav.signIn}
                    </button>
                    <button
