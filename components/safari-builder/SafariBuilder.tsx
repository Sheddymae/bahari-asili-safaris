...
export default function SafariBuilder() {
  ...
  const validateStep = useCallback((): boolean => {
    ...
    if (step === 0) {
      if (!trip.arrivalDate) e.dates = t.safariBuilder.validation.arrivalRequired;
      else if (!trip.departureDate) e.dates = t.safariBuilder.validation.departureRequired;
      else if (new Date(trip.departureDate) <= new Date(trip.arrivalDate)) e.dates = t.safariBuilder.validation.departureAfterArrival;
      if (!trip.startLocation) e.locations = t.safariBuilder.validation.startRequired;
      else if (!trip.endLocation) e.locations = t.safariBuilder.validation.endRequired;
    }
    if (step === 1) {
      if (travellers.adults < 1) e.adults = t.safariBuilder.validation.adultRequired;
      if (travellers.children > 0 && travellers.childrenAges.length !== travellers.children) e.childrenAges = t.safariBuilder.validation.childAgesRequired;
    }
    if (step === 3 && destinationSlugs.length === 0) e.destinations = t.safariBuilder.validation.destinationRequired;
    setErrors(e);
