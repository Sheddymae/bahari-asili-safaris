    ...
    <div className="space-y-8">
      ...
      <div className="grid sm:grid-cols-2 gap-5" role="group" aria-label="Destinations">
        {destinations.map((dest) => {
          ...
          return (
            <button key={dest.slug} type="button" aria-pressed={selected} onClick={() => toggle(dest.slug)} className={`relative text-left rounded-2xl overflow-hidden border-2 transition-all ${selected ? 'border-book shadow-lg' : 'border-transparent hover:border-border'}`}>
              <div className="relative h-40 w-full">
                <Image src={dest.heroImage} alt={dest.name} fill className="object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" />
                {selected && <div className="absolute top-3 right-3 w-7 h-7 bg-book rounded-full flex items-center justify-center"><Check className="w-4 h-4 text-white" /></div>}
              ...
              <div className="bg-white p-4"><p className="font-inter text-xs text-foreground mb-2 line-clamp-2">{dest.tagline}</p><p className="font-inter text-[11px] text-safari-600 font-medium">{dest.wildlifeHighlights[0]}</p></div>
            </button>
          );
        })}
      </div>
