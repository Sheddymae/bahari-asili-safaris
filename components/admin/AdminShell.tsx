...
import { Anchor, BarChart3, Bell, BookOpen, CalendarCheck2, CalendarDays, ChevronDown, ChevronRight, FileSpreadsheet, FileText, Image as ImageIcon, LayoutDashboard, LogOut, Menu, MapPinned, Newspaper, Package, PanelLeftClose, PanelLeftOpen, ReceiptText, ScrollText, Users, WalletCards, Waves, X } from 'lucide-react';
...
export default function AdminShell({ children, title, badge }: { children: React.ReactNode; title: string; badge?: number }) {
  ...
  useEffect(() => { try { window.localStorage.setItem('bahari-admin-sidebar-collapsed', collapsed ? '1' : '0'); } catch { /* ignore */ } }, [collapsed]);
  useEffect(() => { try { window.localStorage.setItem('bahari-admin-sidebar-sections', JSON.stringify(openSections)); } catch { /* ignore */ } }, [openSections]);
  useEffect(() => {
        ...
        <div className="flex-1 overflow-y-auto px-3 pt-5">
          {sections.map(section => {
            ...
            return <div key={section} className="mb-4">
              {collapsed ? <div className="mb-2 border-t border-slate-100 pt-3 first:border-t-0 first:pt-0" aria-hidden="true" /> : <button onClick={() => toggleSection(section as SectionName)} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left hover:bg-slate-50" aria-expanded={sectionOpen}><span className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">{section}</span>{sectionOpen ? <ChevronDown className="h-3.5 w-3.5 text-slate-400" /> : <ChevronRight className="h-3.5 w-3.5 text-slate-400" />}</button>}
              {(sectionOpen || collapsed || hasActive) && <nav className="space-y-1" aria-label={`${section} navigation`}>
                {items.map(({ href, label, icon: Icon }) => <button key={href} onClick={() => { router.push(href); setOpen(false); }} title={collapsed ? label : undefined} aria-label={collapsed ? label : undefined} className={`group flex w-full items-center rounded-xl py-2.5 text-sm font-medium transition ${collapsed ? 'justify-center px-2' : 'gap-3 px-3'} ${active(href) ? 'bg-cyan-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}><Icon className="h-4 w-4 shrink-0" />{!collapsed && <><span className="min-w-0 flex-1 truncate text-left">{label}</span>{label === 'Bookings' && badge ? <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${active(href) ? 'bg-white/20' : 'bg-orange-50 text-orange-600'}`}>{badge}</span> : null}</>}</button>)}
              </nav>}
            </div>;
          })}
        </div>
        <div className="border-t border-slate-100 p-3">
          <button onClick={() => setCollapsed(value => !value)} className="mb-2 hidden w-full items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-500 hover:bg-slate-50 lg:flex" title={collapsed ? 'Expand navigation' : 'Collapse navigation'}>{collapsed ? <PanelLeftOpen className="h-4 w-4" /> : <><PanelLeftClose className="h-4 w-4" />Collapse navigation</>}</button>
