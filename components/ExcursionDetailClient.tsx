...
export default function ExcursionDetailClient({ excursion }: { excursion: Excursion }) {
  ...
  const related = excursions
    .filter((e) => e.category === excursion.category && e.id !== excursion.id)
    .slice(0, 3);
        ...
        <div className="relative h-[50vh] min-h-[360px]">
          ...
          <div className="absolute inset-0 flex flex-col justify-end">
            <div className="max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-10">
              <Link href="/excursions" prefetch className="inline-flex items-center gap-1.5 text-white/80 hover:text-white text-sm font-inter mb-4 transition-colors">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14">
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {content.highlights.map((h, i) => (
              <span key={i} className="bg-sand-50 text-foreground font-inter text-sm px-3 py-1.5 rounded-full border border-sand-200">{h}</span>
            ))}
          </div>
          ...
          {content.whatToExpect.length > 0 && (
            <div className="mb-12">
              ...
              <ul className="space-y-3">
                {content.whatToExpect.map((line, i) => (
                  <li key={i} className="font-inter text-foreground text-base leading-relaxed flex gap-3">
                    <span className="w-1.5 h-1.5 mt-2.5 bg-safari-500 rounded-full flex-shrink-0" />{line}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-sand-50 rounded-2xl p-6">
              ...
              <ul className="space-y-2.5">
                {content.included.map((item, i) => (
                  <li key={i} className="flex items-start gap-2.5 font-inter text-sm text-foreground"><Check className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" />{item}</li>
                ))}
              </ul>
            </div>
            <div className="bg-sand-50 rounded-2xl p-6">
              ...
              <ul className="space-y-2.5">
                {content.notIncluded.map((item, i) => (
                  <li key={i} className="flex items-start gap-2.5 font-inter text-sm text-muted-foreground"><X className="w-4 h-4 text-red-400 mt-0.5 flex-shrink-0" />{item}</li>
                ))}
              </ul>
            </div>
          </div>
          ...
          <div className="mb-12 border border-safari-200 bg-safari-50/40 rounded-2xl p-6">
            ...
            <ul className="space-y-2">
              {content.goodToKnow.map((line, i) => <li key={i} className="font-inter text-sm text-foreground leading-relaxed">{line}</li>)}
            </ul>
          </div>

          <div className="bg-foreground rounded-2xl p-8 sm:p-10 text-center">
            <h3 className="font-poppins font-bold text-2xl text-white mb-3">{ee.askAboutExcursion.replace('this excursion', content.name)}</h3>
            <p className="font-inter text-white/70 text-sm mb-6 max-w-lg mx-auto">{ee.excursionCtaDescription}</p>
            <Link href={`/?book=${excursion.id}`} className="inline-flex items-center justify-center gap-2 bg-safari-500 hover:bg-safari-600 text-white font-poppins font-semibold text-sm px-8 py-3.5 rounded-xl transition-all hover:shadow-md">{ee.requestThisExcursion}</Link>
          </div>
          ...
          {related.length > 0 && (
            <div className="mt-16">
              ...
              <div className="grid sm:grid-cols-3 gap-6">
                {related.map((r) => {
                  const relatedContent = getLocalizedExcursion(r, locale as SupportedLocale);
                  return (
                    <Link key={r.id} href={`/excursions/${r.id}`} prefetch className="group rounded-2xl overflow-hidden border border-border shadow-card hover:shadow-card-hover transition-shadow bg-white">
                      <div className="relative h-32 w-full overflow-hidden"><Image src={r.image} alt={relatedContent.name} fill sizes="33vw" className="object-cover group-hover:scale-105 transition-transform duration-500" /></div>
                      <div className="p-4"><p className="font-poppins font-semibold text-sm text-foreground">{relatedContent.name}</p><p className="font-inter text-xs text-muted-foreground mt-1">{relatedContent.duration}</p></div>
                    </Link>
                  );
                })}
              </div>
