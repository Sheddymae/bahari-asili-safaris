      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {excursions.map(exc => (
            <div
              key={exc.id}
              className="tour-card bg-white border border-border rounded-2xl overflow-hidden shadow-card flex flex-col group"
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-1 flex items-center gap-1">
                  ...
                  <span className="font-inter text-xs font-semibold text-foreground">{exc.duration}</span>
                </div>
                {exc.popular && (
                  <div className="absolute top-3 left-3 bg-safari-500 rounded-full px-3 py-1 shadow-sm">
                    <span className="font-inter font-bold text-[10px] tracking-wide uppercase text-white">{t.excursions.popular}</span>
                  </div>
                )}
              </div>

              <div className="p-5 flex flex-col flex-1">
                <p className="font-inter text-muted-foreground text-sm leading-relaxed mb-4 flex-1">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {exc.highlights.map((h, i) => (
                    <span key={i} className="bg-sand-50 text-foreground font-inter text-xs px-2.5 py-0.5 rounded-full border border-sand-200">
                      {h}
                    </span>
                  ))}
                </div>
