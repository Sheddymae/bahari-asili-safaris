...
export default function ExcursionsSection({ onBook, variant = 'full' }: { onBook: (name?: string) => void; variant?: ExcursionsVariant }) {
  ...
  const homeExcursions = variant === 'home'
    ? [...excursions.filter((exc) => exc.popular), ...excursions.filter((exc) => !exc.popular)].slice(0, 6)
    : excursions;

  return (
    <section id="excursions" className={`${variant === 'home' ? 'py-16 lg:py-20' : 'py-20 lg:py-28'} bg-white`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10 lg:mb-12">
          ...
          <div className="flex flex-col items-start lg:items-end gap-3">
            ...
            {variant === 'home' && <Link href="/excursions" className="font-inter text-sm font-semibold text-ocean-700 hover:text-ocean-800">{t.nav.excursions} →</Link>}
          </div>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
          {homeExcursions.map(exc => {
            ...
            return <div key={exc.id} className="tour-card bg-white border border-border rounded-2xl overflow-hidden shadow-card flex flex-col group">
              <div className={`relative ${variant === 'home' ? 'h-44' : 'h-48'} overflow-hidden`}>
                <Link href={`/excursions/${exc.id}`} aria-label={`${t.excursions.viewExcursion}: ${localized.name}`} prefetch>
                ...
                <div className="absolute bottom-3 left-3 right-3 pointer-events-none"><h3 className="font-poppins font-bold text-white text-base leading-snug">{localized.name}</h3></div>
              ...
              <div className="p-5 flex flex-col flex-1">
                <p className="font-inter text-muted-foreground text-sm leading-relaxed mb-4">{localized.description}</p>
                {variant === 'full' && <div className="flex flex-wrap gap-1.5 mb-4">{localized.highlights.map((h, i) => <span key={i} className="bg-sand-50 text-foreground font-inter text-xs px-2.5 py-0.5 rounded-full border border-sand-200">{h}</span>)}</div>}
                <div className="mt-auto grid grid-cols-2 gap-2">
                  <Link href={`/excursions/${exc.id}`} prefetch className="w-full inline-flex items-center justify-center gap-1.5 border border-border hover:border-safari-400 text-foreground hover:text-safari-700 font-poppins font-semibold text-xs sm:text-sm py-3 rounded-xl transition-all">
                    {t.excursions.viewExcursion}<ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                  <button type="button" onClick={() => onBook(localized.name)} className="w-full flex items-center justify-center gap-1.5 bg-safari-500 hover:bg-safari-600 text-white font-poppins font-semibold text-xs sm:text-sm py-3 rounded-xl transition-all hover:shadow-md">{t.excursions.bookNow}<ChevronRight className="w-4 h-4" /></button>
                </div>
              </div>
            </div>;
          })}
