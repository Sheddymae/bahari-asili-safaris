          ...
          {homeExcursions.map(exc => {
            ...
            return <div key={exc.id} className="tour-card bg-white border border-border rounded-2xl overflow-hidden shadow-card flex flex-col group">
              ...
              <div className="p-5 flex flex-col flex-1">
                <p className="font-inter text-muted-foreground text-sm leading-relaxed mb-4 flex-1 line-clamp-2">{localized.description}</p>
                ...
                <button type="button" onClick={() => onBook(localized.name)} className="w-full flex items-center justify-center gap-2 bg-safari-500 hover:bg-safari-600 text-white font-poppins font-semibold text-sm py-3 rounded-xl transition-all hover:shadow-md">{t.excursions.bookNow}<ChevronRight className="w-4 h-4" /></button>
              </div>
            </div>;
          })}
        </div>
