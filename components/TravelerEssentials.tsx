...
export default function TravelerEssentials() {
  ...
  return (
    <section
      ...
      lang={locale}
      dir={isRTL ? 'rtl' : 'ltr'}
      className="relative overflow-hidden border-y border-sand-200 bg-white py-14 sm:py-16 lg:py-20"
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div key={`traveler-grid-${locale}`} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {content.items.map(({ title, text, href }, index) => {
            const Icon = ESSENTIAL_ICONS[index] ?? MapPinned;
            return (
              <AnimateOnScroll key={`${locale}-${index}-${title}`} direction="up" delay={index * 50}>
