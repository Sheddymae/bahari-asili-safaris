    ...
    <>
      ...
      <main className="overflow-x-hidden">
        <div className="relative h-[50vh] min-h-[360px]">
          ...
          <div className="absolute inset-0 flex flex-col justify-end">
            <div className="max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-10">
              ...
              <p className="font-inter media-text-muted text-base mt-2 max-w-2xl">{content.tagline}</p>
            </div>
          </div>
        </div>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14">
          ...
          <div className="bg-ocean-700 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4"><div><p className="font-poppins font-bold media-text text-lg">{ee.readyToExplore} {content.name}?</p><p className="font-inter media-text-muted text-sm mt-1">{tt.noPricesNote}</p></div><Link href="/tours" prefetch className="flex-shrink-0 bg-safari-500 hover:bg-safari-600 media-text font-poppins font-semibold text-sm px-8 py-3.5 rounded-xl transition-all hover:shadow-md whitespace-nowrap">{tt.bookNow}</Link></div>
        </div>
      </main>
      <Footer />
    </>
