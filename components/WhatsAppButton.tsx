  ...
  return (
    <div className="fixed bottom-5 right-5 z-[9999] flex items-end justify-end pointer-events-none">
      <a
        ...
        className="whatsapp-breathe group relative pointer-events-auto flex h-11 items-center overflow-hidden rounded-full bg-white/10 pl-[11px] pr-[11px] text-white shadow-[0_0_12px_rgba(37,211,102,0.55)] transition-all duration-300 ease-out animate-[whatsapp-breathe_12s_ease-in-out_infinite] hover:pr-5 hover:text-white hover:shadow-[0_0_22px_rgba(37,211,102,0.85)] hover:scale-105 active:scale-95 focus:outline-none focus:ring-4 focus:ring-[#25D366]/40"
      >
        <span className="absolute -inset-1 -z-10 rounded-full bg-[#25D366]/35 blur-md opacity-70 whatsapp-breathe-glow animate-[whatsapp-breathe_12s_ease-in-out_infinite]" />
      ...
      <style jsx>{`
        @keyframes whatsapp-breathe {
          0%, 100% {
            filter: drop-shadow(0 0 4px rgba(37, 211, 102, 0.28));
            opacity: 0.76;
          }
          50% {
            filter: drop-shadow(0 0 10px rgba(37, 211, 102, 0.58));
            opacity: 1;
          }
        }
        @media (prefers-reduced-motion: reduce) {
          .whatsapp-breathe,
          .whatsapp-breathe-glow {
            animation: none;
          }
        }
      `}</style>
