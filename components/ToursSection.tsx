...
const TABS: { key: TabKey; label: string; emoji: string }[] = [
  { key: 'tsavo', label: 'Tsavo', emoji: '🐘' }, { key: 'amboseli', label: 'Amboseli', emoji: '🏔️' }, { key: 'mara', label: 'Masai Mara', emoji: '🦁' }, { key: 'taita', label: 'Taita Hills', emoji: '🦅' }, { key: 'multiday', label: 'Multi-day', emoji: '🗺️' },
];
...
function SafariCard({ safari, t, locale, onBook }: { safari: Safari; t: any; locale: SafariLocale; onBook: (name: string) => void }) {
  ...
  return (
    <article className="bg-white rounded-2xl overflow-hidden shadow-card border border-border flex flex-col h-full">
      <div className="relative h-52"><Image src={safari.image} alt={s.name} fill className="object-cover" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />{safari.popular && <span className="absolute top-3 left-3 bg-safari-500 text-white rounded-full px-3 py-1 font-inter font-bold text-[10px] tracking-wide uppercase">{t.popular}</span>}<div className="absolute top-3 right-3 bg-white/95 rounded-full px-2.5 py-1 flex items-center gap-1"><Star className="w-3 h-3 fill-accent text-accent" /><span className="font-inter font-semibold text-xs">{safari.rating}</span></div><div className="absolute bottom-3 left-4 right-4"><h3 className="font-poppins font-bold text-white text-lg leading-tight">{s.name}</h3><p className="font-inter text-white/85 text-xs mt-1">{s.tagline}</p></div></div>
      <div className="p-5 flex flex-col flex-1">
        ...
        <a href={whatsappHref(s.name)} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center justify-center gap-1.5 text-[#168f45] hover:text-[#0f7437] text-xs font-semibold py-2"><MessageCircle className="w-3.5 h-3.5" />WhatsApp about this safari</a>
      </div>
    </article>
  );
}
...
function CompactSafariCard({ safari, t, locale }: { safari: Safari; t: any; locale: SafariLocale }) {
  const s = getLocalizedSafari(safari, locale);
  return <article className="bg-white rounded-2xl overflow-hidden shadow-card border border-border flex flex-col h-full group">
    <Link href={`/safaris/${safari.id}`} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-safari-400 focus-visible:ring-inset"><div className="relative h-44 sm:h-48"><Image src={safari.image} alt={s.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><div className="absolute top-3 left-3 flex items-center gap-2">{safari.popular && <span className="bg-safari-500 text-white rounded-full px-3 py-1 font-inter font-bold text-[10px] tracking-wide uppercase">{t.popular}</span>}<span className="bg-white/95 rounded-full px-2.5 py-1 flex items-center gap-1"><Star className="w-3 h-3 fill-accent text-accent" /><span className="font-inter font-semibold text-xs">{safari.rating}</span></span></div><div className="absolute bottom-3 left-4 right-4"><h3 className="font-poppins font-bold text-white text-lg leading-tight">{s.name}</h3></div></div></Link>
    <div className="p-4 flex flex-col flex-1"><span className="inline-flex w-fit items-center gap-1.5 bg-sand-50 rounded-full px-3 py-1 font-inter text-xs text-ocean-700 font-semibold"><Clock className="w-3.5 h-3.5" />{safari.days} {t.days} / {safari.nights} {t.nights}</span><Link href={`/safaris/${safari.id}`} className="mt-4 inline-flex items-center justify-center rounded-xl bg-ocean-700 px-4 py-2.5 font-inter text-sm font-semibold text-white transition-colors hover:bg-ocean-800">{t.readMore} →</Link><a href={whatsappHref(s.name)} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center justify-center gap-1.5 text-[#168f45] text-xs font-semibold py-2"><MessageCircle className="w-3.5 h-3.5" />WhatsApp</a></div>
  </article>;
}
