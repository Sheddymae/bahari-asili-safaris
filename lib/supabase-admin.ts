...
/**
 * Returns the public URL a document at `path` will have once uploaded —
 * `getPublicUrl` just builds the URL string, it doesn't check the file
 * exists. Used to embed a QR code pointing at the PDF's own eventual
 * location *inside* that same PDF, generated in one pass before upload.
 */
export function getDocumentPublicUrl(admin: SupabaseClient, path: string): string | null {
  try {
    const { data } = admin.storage.from(DOCUMENTS_BUCKET).getPublicUrl(path);
    return data?.publicUrl || null;
  } catch (err) {
    console.error('getDocumentPublicUrl error:', err);
    return null;
  }
}
