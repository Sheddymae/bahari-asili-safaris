...
export async function generateQuotationPDF(quotation: Quotation, requestedLocale?: Locale, documentUrl?: string): Promise<{ dataUrl: string; base64: string }> {
  ...
  drawBrandLogo(doc, logo, M, 8, 24);
  setFont('bold', 13, PRIMARY); doc.text('BAHARI ASILI SAFARIS', 43, 15);
  setFont('normal', 7.5, MUTED); doc.text(safe(COMPANY.address), 43, 20); doc.text(safe(COMPANY.phone), 43, 24); doc.text(safe(COMPANY.email), 43, 28);
  ...
  doc.setFillColor(...ACCENT); doc.roundedRect(summaryX - 3, y - 3, W - M - summaryX + 3, 12, 2, 2, 'F'); setFont('bold', 9, WHITE); doc.text(Q.total, summaryX, y + 4); doc.text(money(quotation.total_cost), W - M - 3, y + 4, { align: 'right' }); y += 17;
