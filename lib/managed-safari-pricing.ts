...
export interface ManagedDestinationRate {
...
export interface PricingConfig {
  destinations: Partial<Record<SafariTab, ManagedDestinationRate>>;
  currencies: Record<BookingCurrency, CurrencyRate>;
}
...
export function defaultPricingConfig(): PricingConfig {
  ...
  (Object.keys(DESTINATION_RATES) as SafariTab[]).forEach((slug) => {
    ...
    destinations[slug] = {
      accommodationPerNight: r.accommodationPerNight,
      parkFeePerAdultPerDay: r.parkFeePerAdultPerDay,
      parkFeePerChildPerDay: r.parkFeePerChildPerDay,
      guideVehiclePerDay: r.guideVehiclePerDay,
      mealsPerAdultPerDay: r.mealsPerAdultPerDay,
      mealsPerChildPerDay: r.mealsPerChildPerDay,
      pricingMode: 'manual',
      competitorReference: 0,
...
function effectiveRate(rate: ManagedDestinationRate, field: keyof Pick<ManagedDestinationRate, 'accommodationPerNight' | 'parkFeePerAdultPerDay' | 'parkFeePerChildPerDay' | 'guideVehiclePerDay' | 'mealsPerAdultPerDay' | 'mealsPerChildPerDay'>): number {
...
export function buildManagedSafariPlan(input: SafariBuilderInput, currency: BookingCurrency, config: PricingConfig): SafariBuilderResult {
  // Reuse the existing itinerary/destination logic, then replace only the
  // pricing calculation. This keeps the established builder behaviour intact.
  ...
  split.forEach(({ slug, nights: n }) => {
    ...
    const rate: ManagedDestinationRate = managed || {
      ...fallback,
      pricingMode: 'manual',
      competitorReference: 0,
      targetMarkupPercent: 0,
  ...
  // Preserve the existing transparent transfer model.
  let transport = 0;
  const legs = ['start', ...input.destinationSlugs, 'end'] as (SafariTab | 'start' | 'end')[];
  for (let i = 0; i < legs.length - 1; i++) {
    const a = legs[i];
    const b = legs[i + 1];
    transport += a === 'mara' || b === 'mara' ? 16000 : 7000;
  }
  ...
  const kesPricing: PricingBreakdown = {
    accommodation_cost: Math.round(accommodation),
    park_fees: Math.round(parkFees),
    guide_cost: Math.round(guide),
    transport_cost: Math.round(transport),
    meals_cost: Math.round(meals),
    other_costs: other,
    discount,
    tax,
    total_cost: total,
